# The sublibaries are checked for potential dlls
# If they exist, add that sublibrary as a shared one, else as a static one
...
macro(_find_physx_library SUFFIX)
    ...
    find_library(${VAR_NAME}_LIBRARY_RELEASE
            ...
            PATHS ${_PHYSX_RELEASE_PATHS}/lib
            NO_CMAKE_PATH
            NO_DEFAULT_PATH
    )
    ...
    find_library(${VAR_NAME}_LIBRARY_DEBUG
            NAMES ${_${VAR_NAME}_LIBRARY_NAMES}
            PATHS ${_PHYSX_DEBUG_PATHS}/lib
            NO_CMAKE_PATH
            NO_DEFAULT_PATH
    )

    find_library(${VAR_NAME}_DLL_RELEASE
            NAMES ${_${VAR_NAME}_LIBRARY_NAMES}
            PATHS ${_PHYSX_RELEASE_PATHS}/bin
            NO_CMAKE_PATH
            NO_DEFAULT_PATH
    )

    find_library(${VAR_NAME}_DLL_DEBUG
            NAMES ${_${VAR_NAME}_LIBRARY_NAMES}
            PATHS ${_PHYSX_DEBUG_PATHS}/bin
            NO_CMAKE_PATH
            NO_DEFAULT_PATH
    )
    ...
    if(${VAR_NAME}_LIBRARY AND NOT TARGET ${TARGET})
        if(${VAR_NAME}_DLL_RELEASE)
            set(${VAR_NAME}_IS_DLL TRUE)
            add_library(${TARGET} SHARED IMPORTED)
        else()
            set(${VAR_NAME}_IS_DLL FALSE)
        ...
        if(EXISTS "${${VAR_NAME}_LIBRARY_DEBUG}" AND EXISTS "${${VAR_NAME}_LIBRARY_RELEASE}")
            if(${VAR_NAME}_IS_DLL)
                set_target_properties(
                        ${TARGET} PROPERTIES
                        IMPORTED_IMPLIB ${${VAR_NAME}_LIBRARY_RELEASE}
                        IMPORTED_IMPLIB_DEBUG ${${VAR_NAME}_LIBRARY_DEBUG}
                        IMPORTED_IMPLIB_RELEASE ${${VAR_NAME}_LIBRARY_RELEASE}
                        IMPORTED_LOCATION ${${VAR_NAME}_DLL_RELEASE}
                        IMPORTED_LOCATION_DEBUG ${${VAR_NAME}_DLL_DEBUG}
                        IMPORTED_LOCATION_RELEASE ${${VAR_NAME}_DLL_RELEASE}
        ...
        elseif(EXISTS "${${VAR_NAME}_LIBRARY}")
            if(${VAR_NAME}_IS_DLL)
                set_target_properties(
                        ${TARGET} PROPERTIES
                        IMPORTED_IMPLIB ${${VAR_NAME}_LIBRARY}
                        IMPORTED_LOCATION ${${VAR_NAME}_DLL_RELEASE}
                )
            else()
                set_target_properties(
